...
# benchmarks/results/microbench-mb21-${BENCH_RESULT_TAG:-quick}.json.
...
mb_id="MB21"
ext_name="pg_failover_slots"
op_label="wal_write_overhead_pct"
default_rows=10000
...
out="${BENCH_RESULTS_ROOT}/microbench-mb21-${BENCH_RESULT_TAG}.json"
...
JSON
  bench_log "MB21: scaffold result -> ${out} (${note})"
  printf '%s\n' "$(cat "${out}")"
}
...
setup_log="${BENCH_RESULTS_ROOT}/microbench-mb21-${BENCH_RESULT_TAG}.setup.log"
bench_log "${BENCH_RESULTS_ROOT}/microbench-mb21-${BENCH_RESULT_TAG}.bench.log"
...
bench_log_file="${BENCH_RESULTS_ROOT}/microbench-mb21-${BENCH_RESULT_TAG}.bench.log"
...
JSON
bench_log "MB21: measured qps=${qps} elapsed_s=${elapsed_s} rows=${rows} -> ${out}"
printf '%s\n' "$(cat "${out}")"
