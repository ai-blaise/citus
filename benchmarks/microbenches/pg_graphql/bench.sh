...
#
# GraphQL query joining a 10k-row table with FK joins through graphql.resolve.
#
...
# benchmarks/results/microbench-mb14-${BENCH_RESULT_TAG:-quick}.json.
...
out="${BENCH_RESULTS_ROOT}/microbench-mb14-${BENCH_RESULT_TAG}.json"
...
JSON
  bench_log "MB14: scaffold result -> ${out} (${note})"
  printf '%s\n' "$(cat "${out}")"
}
...
setup_log="${BENCH_RESULTS_ROOT}/microbench-mb14-${BENCH_RESULT_TAG}.setup.log"
bench_log "${BENCH_RESULTS_ROOT}/microbench-mb14-${BENCH_RESULT_TAG}.bench.log"
...
bench_log_file="${BENCH_RESULTS_ROOT}/microbench-mb14-${BENCH_RESULT_TAG}.bench.log"
...
JSON
bench_log "MB14: measured qps=${qps} elapsed_s=${elapsed_s} rows=${rows} -> ${out}"
printf '%s\n' "$(cat "${out}")"
