...
`graphql_join_qps` — initial baseline `1500` qps with p95 `8` ms and p99 `30` ms.
...
pg_graphql upstream: ~1.5k qps for 2-table joined GraphQL queries.
...
its first measured `benchmarks/results/microbench-mb14-release.json`.
...
- `bench.sh` — wraps `setup.sql` + `bench.sql` with timing, soft-passes when
  ...
  `benchmarks/results/microbench-mb14-${BENCH_RESULT_TAG}.json`.
...
```sh
BENCH_QUICK=1 bash benchmarks/microbenches/pg_graphql/bench.sh
```
