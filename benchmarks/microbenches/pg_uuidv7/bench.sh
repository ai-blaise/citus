...
# benchmarks/results/microbench-mb19-${BENCH_RESULT_TAG:-quick}.json.
...
mb_id="MB19"
ext_name="pg_uuidv7"
op_label="uuidv7_generations_per_s"
default_rows=100000
...
out="${BENCH_RESULTS_ROOT}/microbench-mb19-${BENCH_RESULT_TAG}.json"
...
JSON
  bench_log "MB19: scaffold result -> ${out} (${note})"
  printf '%s\n' "$(cat "${out}")"
}
...
setup_log="${BENCH_RESULTS_ROOT}/microbench-mb19-${BENCH_RESULT_TAG}.setup.log"
bench_log "${BENCH_RESULTS_ROOT}/microbench-mb19-${BENCH_RESULT_TAG}.bench.log"
...
bench_log_file="${BENCH_RESULTS_ROOT}/microbench-mb19-${BENCH_RESULT_TAG}.bench.log"
...
JSON
bench_log "MB19: measured qps=${qps} elapsed_s=${elapsed_s} rows=${rows} -> ${out}"
printf '%s\n' "$(cat "${out}")"
