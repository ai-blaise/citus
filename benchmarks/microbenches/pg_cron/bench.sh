...
# benchmarks/results/microbench-mb4-${BENCH_RESULT_TAG:-quick}.json.
...
mb_id="MB4"
ext_name="pg_cron"
op_label="job_schedule_overhead_ms"
default_rows=100
...
out="${BENCH_RESULTS_ROOT}/microbench-mb4-${BENCH_RESULT_TAG}.json"
...
JSON
  bench_log "MB4: scaffold result -> ${out} (${note})"
  printf '%s\n' "$(cat "${out}")"
}
...
setup_log="${BENCH_RESULTS_ROOT}/microbench-mb4-${BENCH_RESULT_TAG}.setup.log"
bench_log "${BENCH_RESULTS_ROOT}/microbench-mb4-${BENCH_RESULT_TAG}.bench.log"
...
bench_log_file="${BENCH_RESULTS_ROOT}/microbench-mb4-${BENCH_RESULT_TAG}.bench.log"
...
JSON
bench_log "MB4: measured qps=${qps} elapsed_s=${elapsed_s} rows=${rows} -> ${out}"
printf '%s\n' "$(cat "${out}")"
