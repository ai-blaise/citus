...
# benchmarks/results/microbench-mb9-${BENCH_RESULT_TAG:-quick}.json.
...
mb_id="MB9"
ext_name="postgresql-topn"
op_label="topn_add_agg_ms"
default_rows=100000
...
out="${BENCH_RESULTS_ROOT}/microbench-mb9-${BENCH_RESULT_TAG}.json"
...
JSON
  bench_log "MB9: scaffold result -> ${out} (${note})"
  printf '%s\n' "$(cat "${out}")"
}
...
setup_log="${BENCH_RESULTS_ROOT}/microbench-mb9-${BENCH_RESULT_TAG}.setup.log"
bench_log "${BENCH_RESULTS_ROOT}/microbench-mb9-${BENCH_RESULT_TAG}.bench.log"
...
bench_log_file="${BENCH_RESULTS_ROOT}/microbench-mb9-${BENCH_RESULT_TAG}.bench.log"
...
JSON
bench_log "MB9: measured qps=${qps} elapsed_s=${elapsed_s} rows=${rows} -> ${out}"
printf '%s\n' "$(cat "${out}")"
