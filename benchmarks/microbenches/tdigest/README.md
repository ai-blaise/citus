...
`tdigest_percentile_ms` — initial baseline `100000` qps with p95 `3` ms and p99 `10` ms.
...
tdigest upstream benchmarks: ~100k-row aggregation <1 s.
...
its first measured `benchmarks/results/microbench-mb10-release.json`.
...
- `bench.sh` — wraps `setup.sql` + `bench.sql` with timing, soft-passes when
  ...
  `benchmarks/results/microbench-mb10-${BENCH_RESULT_TAG}.json`.
...
```sh
BENCH_QUICK=1 bash benchmarks/microbenches/tdigest/bench.sh
```
