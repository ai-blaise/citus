...
#
# tdigest_percentile aggregation over 100k numeric samples.
#
...
# benchmarks/results/microbench-mb10-${BENCH_RESULT_TAG:-quick}.json.
...
mb_id="MB10"
ext_name="tdigest"
op_label="tdigest_percentile_ms"
default_rows=100000
...
out="${BENCH_RESULTS_ROOT}/microbench-mb10-${BENCH_RESULT_TAG}.json"
...
JSON
  bench_log "MB10: scaffold result -> ${out} (${note})"
  printf '%s\n' "$(cat "${out}")"
}
...
setup_log="${BENCH_RESULTS_ROOT}/microbench-mb10-${BENCH_RESULT_TAG}.setup.log"
bench_log "${BENCH_RESULTS_ROOT}/microbench-mb10-${BENCH_RESULT_TAG}.bench.log"
...
bench_log_file="${BENCH_RESULTS_ROOT}/microbench-mb10-${BENCH_RESULT_TAG}.bench.log"
...
JSON
bench_log "MB10: measured qps=${qps} elapsed_s=${elapsed_s} rows=${rows} -> ${out}"
printf '%s\n' "$(cat "${out}")"
