...
# benchmarks/results/microbench-mb22-${BENCH_RESULT_TAG:-quick}.json.
...
mb_id="MB22"
ext_name="pg_warm"
op_label="warm_throughput_mb_per_s"
default_rows=100000
...
out="${BENCH_RESULTS_ROOT}/microbench-mb22-${BENCH_RESULT_TAG}.json"
...
JSON
  bench_log "MB22: scaffold result -> ${out} (${note})"
  printf '%s\n' "$(cat "${out}")"
}
...
setup_log="${BENCH_RESULTS_ROOT}/microbench-mb22-${BENCH_RESULT_TAG}.setup.log"
bench_log "${BENCH_RESULTS_ROOT}/microbench-mb22-${BENCH_RESULT_TAG}.bench.log"
...
bench_log_file="${BENCH_RESULTS_ROOT}/microbench-mb22-${BENCH_RESULT_TAG}.bench.log"
...
JSON
bench_log "MB22: measured qps=${qps} elapsed_s=${elapsed_s} rows=${rows} -> ${out}"
printf '%s\n' "$(cat "${out}")"
