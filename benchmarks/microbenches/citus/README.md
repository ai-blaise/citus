...
`distributed_insert_rows_per_s` — initial baseline `30000` qps with p95 `25` ms and p99 `80` ms.
...
Citus docs: distributed INSERT throughput on a 3-worker cluster.
...
its first measured `benchmarks/results/microbench-mb2-release.json`.
...
- `bench.sh` — wraps `setup.sql` + `bench.sql` with timing, soft-passes when
  ...
  `benchmarks/results/microbench-mb2-${BENCH_RESULT_TAG}.json`.
...
```sh
BENCH_QUICK=1 bash benchmarks/microbenches/citus/bench.sh
```
