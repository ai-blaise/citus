...
#
# create_distributed_table + 100k INSERT routed via the coordinator across 3 worker shards.
#
...
# benchmarks/results/microbench-mb2-${BENCH_RESULT_TAG:-quick}.json.
...
mb_id="MB2"
ext_name="citus"
op_label="distributed_insert_rows_per_s"
default_rows=100000
...
out="${BENCH_RESULTS_ROOT}/microbench-mb2-${BENCH_RESULT_TAG}.json"
...
JSON
  bench_log "MB2: scaffold result -> ${out} (${note})"
  printf '%s\n' "$(cat "${out}")"
}
...
setup_log="${BENCH_RESULTS_ROOT}/microbench-mb2-${BENCH_RESULT_TAG}.setup.log"
bench_log "${BENCH_RESULTS_ROOT}/microbench-mb2-${BENCH_RESULT_TAG}.bench.log"
...
bench_log_file="${BENCH_RESULTS_ROOT}/microbench-mb2-${BENCH_RESULT_TAG}.bench.log"
...
JSON
bench_log "MB2: measured qps=${qps} elapsed_s=${elapsed_s} rows=${rows} -> ${out}"
printf '%s\n' "$(cat "${out}")"
