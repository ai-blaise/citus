...
# benchmarks/results/microbench-mb23-${BENCH_RESULT_TAG:-quick}.json.
...
mb_id="MB23"
ext_name="pgcrypto"
op_label="pgp_sym_encrypt_rows_per_s"
default_rows=10000
...
out="${BENCH_RESULTS_ROOT}/microbench-mb23-${BENCH_RESULT_TAG}.json"
...
JSON
  bench_log "MB23: scaffold result -> ${out} (${note})"
  printf '%s\n' "$(cat "${out}")"
}
...
setup_log="${BENCH_RESULTS_ROOT}/microbench-mb23-${BENCH_RESULT_TAG}.setup.log"
bench_log "${BENCH_RESULTS_ROOT}/microbench-mb23-${BENCH_RESULT_TAG}.bench.log"
...
bench_log_file="${BENCH_RESULTS_ROOT}/microbench-mb23-${BENCH_RESULT_TAG}.bench.log"
...
JSON
bench_log "MB23: measured qps=${qps} elapsed_s=${elapsed_s} rows=${rows} -> ${out}"
printf '%s\n' "$(cat "${out}")"
