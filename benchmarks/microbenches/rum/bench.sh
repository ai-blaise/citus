...
# MB26: rum microbench.
#
# RUM full-text index build + FTS lookup on 100k documents.
#
...
# benchmarks/results/microbench-mb26-${BENCH_RESULT_TAG:-quick}.json.
...
mb_id="MB26"
ext_name="rum"
op_label="rum_fts_index_build_lookup_qps"
default_rows=100000
...
out="${BENCH_RESULTS_ROOT}/microbench-mb26-${BENCH_RESULT_TAG}.json"
...
JSON
  bench_log "MB26: scaffold result -> ${out} (${note})"
  printf '%s\n' "$(cat "${out}")"
}
...
setup_log="${BENCH_RESULTS_ROOT}/microbench-mb26-${BENCH_RESULT_TAG}.setup.log"
bench_log "${BENCH_RESULTS_ROOT}/microbench-mb26-${BENCH_RESULT_TAG}.bench.log"
...
bench_log_file="${BENCH_RESULTS_ROOT}/microbench-mb26-${BENCH_RESULT_TAG}.bench.log"
...
JSON
bench_log "MB26: measured qps=${qps} elapsed_s=${elapsed_s} rows=${rows} -> ${out}"
printf '%s\n' "$(cat "${out}")"
