...
`rum_fts_index_build_lookup_qps` — initial baseline `4000` qps with p95 `5` ms and p99 `20` ms.
...
rum upstream: ~4k qps for FTS lookups on 100k docs.
...
its first measured `benchmarks/results/microbench-mb26-release.json`.
...
- `bench.sh` — wraps `setup.sql` + `bench.sql` with timing, soft-passes when
  ...
  `benchmarks/results/microbench-mb26-${BENCH_RESULT_TAG}.json`.
...
```sh
BENCH_QUICK=1 bash benchmarks/microbenches/rum/bench.sh
```
