...
# benchmarks/results/microbench-mb8-${BENCH_RESULT_TAG:-quick}.json.
...
mb_id="MB8"
ext_name="postgresql-hll"
op_label="hll_add_agg_ms"
default_rows=100000
...
out="${BENCH_RESULTS_ROOT}/microbench-mb8-${BENCH_RESULT_TAG}.json"
...
JSON
  bench_log "MB8: scaffold result -> ${out} (${note})"
  printf '%s\n' "$(cat "${out}")"
}
...
setup_log="${BENCH_RESULTS_ROOT}/microbench-mb8-${BENCH_RESULT_TAG}.setup.log"
bench_log "${BENCH_RESULTS_ROOT}/microbench-mb8-${BENCH_RESULT_TAG}.bench.log"
...
bench_log_file="${BENCH_RESULTS_ROOT}/microbench-mb8-${BENCH_RESULT_TAG}.bench.log"
...
JSON
bench_log "MB8: measured qps=${qps} elapsed_s=${elapsed_s} rows=${rows} -> ${out}"
printf '%s\n' "$(cat "${out}")"
