...
`trigram_similarity_qps` — initial baseline `5000` qps with p95 `5` ms and p99 `20` ms.
...
pg_trgm upstream: ~5k qps on 100k-row trigram index.
...
its first measured `benchmarks/results/microbench-mb24-release.json`.
...
- `bench.sh` — wraps `setup.sql` + `bench.sql` with timing, soft-passes when
  ...
  `benchmarks/results/microbench-mb24-${BENCH_RESULT_TAG}.json`.
...
```sh
BENCH_QUICK=1 bash benchmarks/microbenches/pg_trgm/bench.sh
```
