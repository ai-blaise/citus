...
#
# Trigram similarity lookups against a GIN-trigram index on 100k rows.
#
...
# benchmarks/results/microbench-mb24-${BENCH_RESULT_TAG:-quick}.json.
...
mb_id="MB24"
ext_name="pg_trgm"
op_label="trigram_similarity_qps"
default_rows=100000
...
out="${BENCH_RESULTS_ROOT}/microbench-mb24-${BENCH_RESULT_TAG}.json"
...
JSON
  bench_log "MB24: scaffold result -> ${out} (${note})"
  printf '%s\n' "$(cat "${out}")"
}
...
setup_log="${BENCH_RESULTS_ROOT}/microbench-mb24-${BENCH_RESULT_TAG}.setup.log"
bench_log "${BENCH_RESULTS_ROOT}/microbench-mb24-${BENCH_RESULT_TAG}.bench.log"
...
bench_log_file="${BENCH_RESULTS_ROOT}/microbench-mb24-${BENCH_RESULT_TAG}.bench.log"
...
JSON
bench_log "MB24: measured qps=${qps} elapsed_s=${elapsed_s} rows=${rows} -> ${out}"
printf '%s\n' "$(cat "${out}")"
