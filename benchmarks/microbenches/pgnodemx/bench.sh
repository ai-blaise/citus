...
# benchmarks/results/microbench-mb11-${BENCH_RESULT_TAG:-quick}.json.
...
mb_id="MB11"
ext_name="pgnodemx"
op_label="pgnodemx_cpu_invocation_us"
default_rows=1000
...
out="${BENCH_RESULTS_ROOT}/microbench-mb11-${BENCH_RESULT_TAG}.json"
...
JSON
  bench_log "MB11: scaffold result -> ${out} (${note})"
  printf '%s\n' "$(cat "${out}")"
}
...
setup_log="${BENCH_RESULTS_ROOT}/microbench-mb11-${BENCH_RESULT_TAG}.setup.log"
bench_log "${BENCH_RESULTS_ROOT}/microbench-mb11-${BENCH_RESULT_TAG}.bench.log"
...
bench_log_file="${BENCH_RESULTS_ROOT}/microbench-mb11-${BENCH_RESULT_TAG}.bench.log"
...
JSON
bench_log "MB11: measured qps=${qps} elapsed_s=${elapsed_s} rows=${rows} -> ${out}"
printf '%s\n' "$(cat "${out}")"
