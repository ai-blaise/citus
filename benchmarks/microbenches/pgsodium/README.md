...
`libsodium_encrypt_rows_per_s` — initial baseline `5000` qps with p95 `2` ms and p99 `10` ms.
Canonical workload size: `1000` rows (quick mode runs at one-tenth that
size to fit the CI smoke under the shared 60 s budget).

## Baseline source

pgsodium upstream micro-benches: ~5k crypto_secretbox ops/s.
...
its first measured `benchmarks/results/microbench-mb7-release.json`.
...
- `bench.sh` — wraps `setup.sql` + `bench.sql` with timing, soft-passes when
  ...
  `benchmarks/results/microbench-mb7-${BENCH_RESULT_TAG}.json`.
...
```sh
BENCH_QUICK=1 bash benchmarks/microbenches/pgsodium/bench.sh
```
