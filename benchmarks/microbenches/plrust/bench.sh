...
#
# Call a trivial plrust function 10k times; reports per-call overhead.
#
...
# benchmarks/results/microbench-mb17-${BENCH_RESULT_TAG:-quick}.json.
...
mb_id="MB17"
ext_name="plrust"
op_label="plrust_function_call_us"
default_rows=10000
...
out="${BENCH_RESULTS_ROOT}/microbench-mb17-${BENCH_RESULT_TAG}.json"
...
JSON
  bench_log "MB17: scaffold result -> ${out} (${note})"
  printf '%s\n' "$(cat "${out}")"
}
...
setup_log="${BENCH_RESULTS_ROOT}/microbench-mb17-${BENCH_RESULT_TAG}.setup.log"
bench_log "${BENCH_RESULTS_ROOT}/microbench-mb17-${BENCH_RESULT_TAG}.bench.log"
...
bench_log_file="${BENCH_RESULTS_ROOT}/microbench-mb17-${BENCH_RESULT_TAG}.bench.log"
...
JSON
bench_log "MB17: measured qps=${qps} elapsed_s=${elapsed_s} rows=${rows} -> ${out}"
printf '%s\n' "$(cat "${out}")"
