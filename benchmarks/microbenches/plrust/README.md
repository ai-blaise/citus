...
`plrust_function_call_us` — initial baseline `200000` qps with p95 `0.1` ms and p99 `0.5` ms.
...
plrust upstream: ~5us per call vs ~50us plpgsql.
...
its first measured `benchmarks/results/microbench-mb17-release.json`.
...
- `bench.sh` — wraps `setup.sql` + `bench.sql` with timing, soft-passes when
  ...
  `benchmarks/results/microbench-mb17-${BENCH_RESULT_TAG}.json`.
...
```sh
BENCH_QUICK=1 bash benchmarks/microbenches/plrust/bench.sh
```
