...
# benchmarks/results/microbench-mb18-${BENCH_RESULT_TAG:-quick}.json.
...
mb_id="MB18"
ext_name="plv8"
op_label="plv8_function_call_us"
default_rows=10000
...
out="${BENCH_RESULTS_ROOT}/microbench-mb18-${BENCH_RESULT_TAG}.json"
...
JSON
  bench_log "MB18: scaffold result -> ${out} (${note})"
  printf '%s\n' "$(cat "${out}")"
}
...
setup_log="${BENCH_RESULTS_ROOT}/microbench-mb18-${BENCH_RESULT_TAG}.setup.log"
bench_log "${BENCH_RESULTS_ROOT}/microbench-mb18-${BENCH_RESULT_TAG}.bench.log"
...
bench_log_file="${BENCH_RESULTS_ROOT}/microbench-mb18-${BENCH_RESULT_TAG}.bench.log"
...
JSON
bench_log "MB18: measured qps=${qps} elapsed_s=${elapsed_s} rows=${rows} -> ${out}"
printf '%s\n' "$(cat "${out}")"
