...
#
# 100k doc INSERT + BM25 index build + 1k BM25 lookups. Reports three numbers.
#
...
# benchmarks/results/microbench-mb13-${BENCH_RESULT_TAG:-quick}.json.
...
mb_id="MB13"
ext_name="pg_search"
op_label="bm25_insert_index_lookup_qps"
default_rows=100000
...
out="${BENCH_RESULTS_ROOT}/microbench-mb13-${BENCH_RESULT_TAG}.json"
...
JSON
  bench_log "MB13: scaffold result -> ${out} (${note})"
  printf '%s\n' "$(cat "${out}")"
}
...
setup_log="${BENCH_RESULTS_ROOT}/microbench-mb13-${BENCH_RESULT_TAG}.setup.log"
bench_log "${BENCH_RESULTS_ROOT}/microbench-mb13-${BENCH_RESULT_TAG}.bench.log"
...
bench_log_file="${BENCH_RESULTS_ROOT}/microbench-mb13-${BENCH_RESULT_TAG}.bench.log"
...
JSON
bench_log "MB13: measured qps=${qps} elapsed_s=${elapsed_s} rows=${rows} -> ${out}"
printf '%s\n' "$(cat "${out}")"
