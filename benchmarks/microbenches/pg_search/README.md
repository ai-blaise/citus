...
`bm25_insert_index_lookup_qps` — initial baseline `3000` qps with p95 `5` ms and p99 `20` ms.
...
ParadeDB benchmarks: BM25 lookup ~3k qps on 100k docs.
...
its first measured `benchmarks/results/microbench-mb13-release.json`.
...
- `bench.sh` — wraps `setup.sql` + `bench.sql` with timing, soft-passes when
  ...
  `benchmarks/results/microbench-mb13-${BENCH_RESULT_TAG}.json`.
...
```sh
BENCH_QUICK=1 bash benchmarks/microbenches/pg_search/bench.sh
```
