...
#
# 100k-row insert across 7 days into a hypertable with compression after the workload completes.
#
# Reads :row_count from BENCH_ROW_COUNT (default 100000; quick mode falls
# back to a tenth of that). Emits a single-line JSON record to stdout and to
# benchmarks/results/microbench-mb1-${BENCH_RESULT_TAG:-quick}.json.
...
op_label="hypertable_insert_rows_per_s"
default_rows=100000
...
out="${BENCH_RESULTS_ROOT}/microbench-mb1-${BENCH_RESULT_TAG}.json"
...
JSON
  bench_log "MB1: scaffold result -> ${out} (${note})"
  printf '%s\n' "$(cat "${out}")"
}
...
setup_log="${BENCH_RESULTS_ROOT}/microbench-mb1-${BENCH_RESULT_TAG}.setup.log"
bench_log "${BENCH_RESULTS_ROOT}/microbench-mb1-${BENCH_RESULT_TAG}.bench.log"
...
bench_log_file="${BENCH_RESULTS_ROOT}/microbench-mb1-${BENCH_RESULT_TAG}.bench.log"
...
JSON
bench_log "MB1: measured qps=${qps} elapsed_s=${elapsed_s} rows=${rows} -> ${out}"
printf '%s\n' "$(cat "${out}")"
