...
`hypertable_insert_rows_per_s` — initial baseline `50000` qps with p95 `20` ms and p99 `50` ms.
...
TigerData docs: bulk insert >50k rows/s on 1 vCPU.
...
its first measured `benchmarks/results/microbench-mb1-release.json`.
...
- `bench.sh` — wraps `setup.sql` + `bench.sql` with timing, soft-passes when
  ...
  `benchmarks/results/microbench-mb1-${BENCH_RESULT_TAG}.json`.
...
```sh
BENCH_QUICK=1 bash benchmarks/microbenches/timescaledb/bench.sh
```
