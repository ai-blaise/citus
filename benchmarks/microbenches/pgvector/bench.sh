...
# benchmarks/results/microbench-mb3-${BENCH_RESULT_TAG:-quick}.json.
...
mb_id="MB3"
ext_name="pgvector"
op_label="ivfflat_insert_then_lookup_qps"
default_rows=1000
...
out="${BENCH_RESULTS_ROOT}/microbench-mb3-${BENCH_RESULT_TAG}.json"
...
JSON
  bench_log "MB3: scaffold result -> ${out} (${note})"
  printf '%s\n' "$(cat "${out}")"
}
...
setup_log="${BENCH_RESULTS_ROOT}/microbench-mb3-${BENCH_RESULT_TAG}.setup.log"
bench_log "${BENCH_RESULTS_ROOT}/microbench-mb3-${BENCH_RESULT_TAG}.bench.log"
...
bench_log_file="${BENCH_RESULTS_ROOT}/microbench-mb3-${BENCH_RESULT_TAG}.bench.log"
...
JSON
bench_log "MB3: measured qps=${qps} elapsed_s=${elapsed_s} rows=${rows} -> ${out}"
printf '%s\n' "$(cat "${out}")"
