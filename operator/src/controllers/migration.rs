...
//!
//! Runs each `MigrationSpec` through the schema-job reconcile plan and the
//! gh-ost-style state machine boundary defined in `crate::crds::migration`.
...
async fn reconcile(
    ...
    info!(
        ...
        "Migration reconciled in bounded dry-run/apply contract"
    );
...
fn current_phase(status: &Option<MigrationStatus>) -> MigrationPhase {
    match status
        .as_ref()
        .map(|status| normalize_phase(&status.phase))
        .as_deref()
    {
        Some("write_only") => MigrationPhase::WriteOnly,
