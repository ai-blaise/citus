...
use ai_blaise_citus_operator::{
    ...
    SidecarEndpointCandidate, SidecarEndpointRetargetPlan, SidecarSpec, SidecarType,
...
fn print_usage() {
    println!(
        "usage: operator [serve|run-canonical|run-reconcile-plans|run-endpointslice-retarget-canonical]"
    );
}
