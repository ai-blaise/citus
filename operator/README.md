...
available, starts kube-rs watch loops for implemented controllers: `CitusCluster`,
`Migration`, `Tenant`, `Region`, `SurvivalGoal`, `Backup`, `Hypertable`,
`Federation`, `SearchIndex`, `Webhook`, `Function`, `ScheduledRepack`,
`ConflictPolicy`, and `Sidecar`. The controllers validate CR shape and build
deterministic apply plans; live child-resource mutation, SQL execution, outbound
HTTP delivery, edge-function execution, external warehouse connectivity,
distributed search query fanout, and `.status` writes remain alpha until they
have cluster evidence.
