...
**Overlay**: `pool/src/runtime.rs`, `sidecar/auth`
**Status**: alpha
**Since**: unreleased
**Upstream Citus equivalent**: none
**Bundled extension dep**: none

**Summary**: Defines pool-side token introspection cache sizing and TTL; the
auth sidecar now also has an in-process introspection/JTI cache used by its
`/auth/introspect`, `/auth/verify`, and `/auth/logout` endpoints.
