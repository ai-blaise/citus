...
**Overlay**: `operator/src/crds/backup.rs`, `operator/src/reconcile/backup.rs`,
`sidecar/backup`
**Status**: production-ready
**Since**: unreleased
**Upstream Citus equivalent**: none
...
**Summary**: Validates the backup encryption contract, materializes the
sidecar WAL-G encryption environment with `WALG_GPG_KEY_ID` before accepting
encrypted backup work, and emits a deterministic operator `KmsBinding` apply
step when a Backup CR specifies a non-empty KMS reference.
...
Production evidence: VM proof for Reconcilers Batch A runs `cargo test -p
ai_blaise_citus_operator` and `ci/ai-blaise/operator-reconcilers-batch-a-smoke.sh`.
The backup reconciler validates non-empty KMS references and emits a distinct
`KmsBinding` apply step into the sidecar configuration plan; unencrypted plans
skip that step deterministically.

**Current boundary**: The production-ready claim covers sidecar environment
validation, encrypted backup orchestration, and deterministic Backup CR
KMS-binding plan construction. It does not prove hardware KMS, External
Secrets, cloud IAM, key rotation, Kubernetes child-resource application, or
encrypted object-store archives with real WAL-G credentials.
...
**Overlay**: `pool/src/runtime.rs`, `pool/src/auth_cache.rs`, `sidecar/auth`
**Status**: alpha
**Since**: unreleased
**Upstream Citus equivalent**: none
**Bundled extension dep**: none
...
revocation, and verified-claim validation; the auth sidecar now also has an
in-process introspection/JTI cache used by its `/auth/introspect`,
`/auth/verify`, and `/auth/logout` endpoints.
