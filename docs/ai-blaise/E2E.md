...
2. preload Citus and TimescaleDB together
3. configure `citus.cohabit_extensions = 'timescaledb'`
4. create a `Hypertable` spec with compression, retention, and CAGG policy
5. map that spec through the operator reconciler into companion plans
6. record the gates the database-backed runner must prove
