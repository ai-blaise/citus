...
`benchmarks/` (TPC-C, sysbench, Timescale ingest, chaos) and the CI smoke
runner is `ci/ai-blaise/benchmark-smoke.sh`. Promotion to production-ready
requires entries in `docs/ai-blaise/PRODUCTION_READINESS_AUDIT.md` and a live
3-worker Citus cluster recording.
