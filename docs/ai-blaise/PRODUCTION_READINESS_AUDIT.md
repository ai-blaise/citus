...
`SC7` sidecar HA retarget primitive is production-ready only for shared
library config parsing, health-aware deterministic selection, failure retarget,
drain exclusion, and generation-tracked reload behavior under focused VM unit
and canonical-runner evidence; Kubernetes EndpointSlice retargeting,
HPA/PDB/chart rendering, NetworkPolicy enforcement, and cross-region failover
execution remain alpha. The other 148 feature headings remain
