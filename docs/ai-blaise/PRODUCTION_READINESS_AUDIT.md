...
The current feature inventory contains 274 source `FEATURE:` markers and 274
...
plan-builder helpers, plus `SC7` for the operator/sidecar EndpointSlice retarget
contract that deterministically renders and validates a selected sidecar
EndpointSlice plus Service merge patch and fails closed with an empty
EndpointSlice when no sidecar is selected, while live EndpointSlice watches,
Service patch application, leader election, and cross-region failover remain
alpha, plus `MCP4` for the narrow `tools/citus-mcp` read-only
