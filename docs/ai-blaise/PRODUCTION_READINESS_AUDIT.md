...
- The Raft/HLC/transaction-status triad now has executable sidecar runtime
  ...
  finalize, and modeled fast-path step count; `schema-txn-runtime-smoke.sh`
  drives the real txn-status HTTP server through stage -> wait -> ack ->
  commit with malformed/unknown-field rejection; and `sql-extension-smoke.sh`
...
- The schema-job sidecar now has an explicit runtime-boundary smoke for the
  ...
  binary's canonical worker output, controller advance/wait/rollback output,
