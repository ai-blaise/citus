...
L13 MotherDuck connector live evidence from 2026-05-26 promotes `FEATURE: L13` from alpha to production-ready for the MotherDuck binding contract + deterministic session accounting + opt-in token fail-closed behavior scope. The new `ci/ai-blaise/l13-motherduck-connector-live-smoke.sh` verifies the MotherDuckConnector struct + impl exists in sidecar/analytical/src/lib.rs, runs the analytical sidecar runtime canonical and asserts motherduck_db=analytics + motherduck_sessions=1, asserts external_io_attempted=false (proving fail-closed without a live MotherDuck token), and emits the evidence row to `artifacts/l13-motherduck-evidence.tsv`. The L13 claim is bounded to the binding contract + accounting + fail-closed scope; live MotherDuck cloud session execution against motherduck.com requires external motherduck_token + outbound cloud connectivity and stays alpha-deferred under the same L13 contract surface.

Upstream rebase 2026-05-26 folded five `citusdata/citus` `main` commits onto `bootstrap-v2` (base `4d54b11bb` -> head `dee8ec140`): #8587 object-ownership enforcement for citus-internal UDFs, #8497 NOT (x IS DISTINCT FROM y) recursive-planning correctness, #8498 COLLATE-with-type-cast distributed-query correctness, #8593 SortList short-circuit, #8592 README grammar. Production-ready end-to-end integration is proven on `experiment-playground-plus-2` (gcloud blaise-478114, Debian 12 + Docker 20.10, 48 cores) by `ci/ai-blaise/upstream-rebase-2026-05-25-integration-smoke.sh` under `REQUIRE_DOCKER=1 RUN_BUNDLE1_FIX_VERIFICATION=1`. Phase 2 boots a `citusdata/citus:14.0.0-pg17` (pre-cherry-pick stable) coordinator + worker on a docker network, calls `master_add_node`, and runs the new regression-test SQL patterns against it: #8497 TLP returns `original=288 true=224 false=0` (sum 224 != 288 confirms the bug), #8498 GROUP BY `(...::VARCHAR COLLATE "C")` errors with the documented `attribute N of type record has wrong type`. Phase 3 rebuilds `bundle1-final-light` from in-tree cherry-picked source with the full ai-blaise customization stack (citus + timescaledb + ai_blaise_citus + vector + pg_cron + age + pgaudit + pgauditlogtofile + pg_failover_slots + pgnodemx + 14 SQL-only extensions), boots the same coordinator+worker topology against it, and runs the identical SQL patterns: #8497 TLP returns `original=288 true=224 false=64` (sum 288 == 288, correct), #8498 GROUP BY succeeds, #8587 ownership remains enforced. Evidence row in `artifacts/upstream-rebase-2026-05-25-integration-evidence.tsv`; the row carries both `upstream_*` and `fork_*` columns for each fix so the bug -> fix delta is machine-readable. The claim is bounded to: source-level cherry-pick presence, multi-node Phase 2 bug reproduction on stable upstream, and Phase 3 fix verification against the rebuilt bundle1 image. `docs/ai-blaise/UPSTREAM_SYNC.md` status snapshot is bumped to 2026-05-26 with the same five-commit list.
...
parse in `pool/src/trace_tap.rs::tap_startup_message`.
`pool/src/pipeline.rs::ExtendedFrame` now wraps the codec's typed frames
instead of synthetic shapes, and `ExtendedPipelineBuffer::append_wire_frame`
decodes raw wire bytes into the buffer between `Sync` flushes. The new
`ci/ai-blaise/pool-extended-query-pipeline-live-smoke.sh` boots `postgres:17`
on a host-bound port and runs `pool/wire/examples/pipeline_live_smoke.rs`,
which uses only the codec to drive a real
`Parse`/`Bind`/`Describe`/`Execute`/`Sync` pipeline (asserts
`ParseComplete` -> `BindComplete` -> `DataRow(sum=42)` -> `CommandComplete` ->
`ReadyForQuery=I`) and a deterministic-failure pipeline with a malformed
