...
The runtime accepts wal2json frames over `serve-runtime` (`POST /ingest`), exposes `/healthz`, `/readyz`, `/metrics`, `/state`, `/streams`, and `/dlq`, applies PII anonymization before encoding sink frames, advances checkpoint/ack state after dispatch, and can bridge events to realtime over TCP or `unix:/path.sock`.

Implemented surfaces:

- `source`: `LogicalReplicationClient`, `ReplicationFrame`, wal2json decoder, pgoutput logical-view decoder, checkpoint/ack tracking.
